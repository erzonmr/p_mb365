...
Plataforma web responsive construida con **Astro + React + TailwindCSS**, diseñada como un **espacio de lectura bíblica editorial**, no como una aplicación móvil ni un blog tradicional.
...
- Cada capítulo genera su propio HTML estático (SSR) para compartir y posicionar en buscadores.
...
### Principio rector
> La web vive en el navegador. El contenido bíblico viene de la API, los planes y configuración desde JSON externos, y la experiencia personal se guarda localmente. Cada capítulo es una URL compartible con su propio HTML semántico.
...
| Hosting | Vercel |
| API bíblica externa (principal + respaldo) | Confirmado |
| Datos JSON externos en GitHub + jsDelivr | Confirmado |
| Solo versiones en español | Confirmado |
| Almacenamiento local: localStorage / IndexedDB (fase inicial) | Confirmado |
| Respaldo en backend real (Supabase) | Fase posterior (roadmap) |
| PWA | **Descartada explícitamente** |
| "Instalación tipo app" | **Descartada** — solo uso directo en navegador |
| Lógica de año bisiesto | Manejada en JS, no en JSON |
| SEO por capítulo | SSR obligatorio para cada ruta de lectura |

### Responsabilidades por capa
| Capa | Función |
|---|---|
| **Astro (SSR/SSG)** | Layout global, rutas, meta-tags por página, renderizado estático de capítulos, SEO |
| **React (islands)** | Selectores, buscador, calendario, formularios de notas, toggle de tema |
| **TailwindCSS** | Sistema de diseño editorial, paleta de color, tipografía, responsive |
| **JSON externos** | Versiones, planes, versículos diarios, configuración general |
| **localStorage / IndexedDB** | Datos personales del usuario (progreso, notas, favoritos, configuración) |
| **API bíblica** | Texto bíblico dinámico por referencia |
...
- Cachear respuestas en memoria durante la sesión.
- Reintentar con la API alterna en caso de error de red.
...
| Archivo | Propósito |
|---|---|
| `versions.json` | Catálogo de versiones bíblicas habilitadas |
...
| `/leer-hoy` | Leer hoy | Lectura diaria del plan |
| `/planes` | Planes de lectura | Catálogo y selección de planes |
| `/plan-completo` | Plan completo | Calendario y navegación del plan |
| `/biblia` | Biblia | Lectura libre por libro/capítulo |
| `/buscar` | Buscar | Búsqueda de palabras/frases |
| `/mi-espacio` | Mi espacio | Panel personal del usuario |
| `/acerca` | Acerca del proyecto | Información y visión |
...
### 11.5 Plan completo (`/plan-completo`)
...
### 11.8 Mi espacio (`/mi-espacio`) — **página central del proyecto**
...
### 11.9 Acerca del proyecto (`/acerca`)
...
### 11.10 Preguntas frecuentes (`/preguntas-frecuentes`)
...
- Layout global y rutas.
- Meta-tags, Open Graph y SEO por página.
- SSR de capítulos bíblicos para que cada URL tenga HTML real.
- Generación de sitemap.
- Integración limpia de scripts y estilos.

### 14.2 Responsabilidades de JavaScript / React
- Gestión del estado global (plan activo, progreso, preferencias).
- Consultas a API con fallback.
- Renderizado de lecturas y versículos en cliente (cuando sea interactivo).
...
| `/leer-hoy` | `src/pages/leer-hoy.astro` |
| `/planes` | `src/pages/planes.astro` |
| `/plan-completo` | `src/pages/plan-completo.astro` |
| `/biblia` | `src/pages/biblia.astro` |
| `/buscar` | `src/pages/buscar.astro` |
| `/mi-espacio` | `src/pages/mi-espacio.astro` |
| `/acerca` | `src/pages/acerca.astro` |
...
### 15.4 Datos del usuario (localStorage / IndexedDB)
```json
{
  "version": "1.0",
  "lastSync": "2026-04-20",
...
### 16.1 Fase 1 — localStorage + IndexedDB (MVP)
**Ventajas:**
...
- **localStorage** para configuración ligera (tema, versión preferida, plan activo).
- **IndexedDB** (vía Dexie.js) para notas largas, favoritos e historial completo, evitando el límite de ~5 MB de localStorage.
...
**Opción recomendada:** **Supabase** — PostgreSQL, Auth listo, Row Level Security, free tier generoso, y fácil integración desde un sitio estático.

### 16.4 Estrategia de migración
Cuando se active el backend, el almacenamiento local se conservará como caché offline. El flujo será:
1. Al iniciar sesión, se descarga el estado del backend.
2. Los cambios locales se sincronizan al backend en segundo plano.
3. Si no hay sesión, funciona como hoy (solo local).
...
- Transiciones suaves entre páginas (View Transitions de Astro, opcional).
...
- Cada capítulo leído tiene una URL limpia que se puede compartir por WhatsApp/Telegram y se ve bien en preview (OpenGraph).
...
> **Mi Biblia 365** es una plataforma web responsive construida con Astro, que permite a los cristianos leer la Biblia mediante múltiples planes y versiones en español, apoyándose en una API bíblica externa con respaldo, datos JSON servidos por CDN, y un espacio personal completo para gestionar progreso, favoritos y notas. El proyecto no requiere registro en su primera fase, funciona directamente en el navegador sin pretender ser una app nativa, y está diseñado con el propósito pastoral de acompañar al creyente en su perseverancia devocional.

---

## 22. Próximos pasos recomendados

1. **Validar API principal hoy mismo** con una prueba mínima (1 página Astro que consuma RV60 y renderice Juan 3:16 vía SSR).
2. **Crear el repositorio GitHub** con estructura de carpetas Astro definida.
...
*Documento de producto. Versión 1.0 — Web Responsive. Abril 2026.*
