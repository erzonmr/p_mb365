...
La identidad visual — paleta orgánica, tipografía editorial dual (Newsreader + Manrope), espacio negativo generoso, capas tonales sin bordes 1px — está definida en los prototipos de la carpeta `prototipo/` y en el documento `prototipo/DESIGN.md`. Todo el sistema de diseño del Santuario Editorial se conserva íntegramente, aplicado bajo la marca Mi Biblia 365.
...
- Gestiona progreso personal, favoritos, notas y **reflexiones del Scriptorio** (diario devocional).
...
- Incluye **sistema de logros** (rachas, hitos de libros completados, medallas de perseverancia) para acompañar sin presionar.
- Cada capítulo genera su propio HTML semántico (SSR) para compartir y posicionar en buscadores.
...
- Guardar favoritos, tomar notas y escribir reflexiones devocionales.
...
- PWA con prompts de instalación.

### Principio rector
> La web vive en el navegador. El contenido bíblico viene de la API, los planes y configuración desde JSON externos (carpeta `json/`), y la experiencia personal se guarda localmente. Cada capítulo es una URL compartible con meta-tags estables generados desde nuestros datos, no dependiendo de la disponibilidad de la API de texto.
...
|---|---|---|
| Framework | **Astro 6** (Q2 2026) | Content-site por excelencia. Zero JS by default, SSR híbrido, View Transitions, CSP automático. |
...
### 10.2 Fuente
Archivo: `json/versiculos-diarios.json` (366 entradas).
...
### 12.1 Menú principal (escritorio y tablet)
Navegación horizontal en **header sticky** superior. En tablet, colapsa en menú hamburguesa.
...
### 12.2 Navegación móvil (bottom nav)
En móvil (< 768px), se usa una **barra de navegación inferior de 4 items** para optimizar el alcance del pulgar en uso una mano:

- **Inicio** (icono: `home_max`)
- **Leer** (icono: `book_5`) — activo por defecto
- **Planes** (icono: `explore`)
- **Mi espacio** (icono: `insights`)

**Nota importante:** Esta bottom nav es una optimización táctil de web responsive, **no una app nativa**. No hay PWA, no hay prompt de instalación, no se oculta la barra de direcciones del navegador. Es una web que se navega cómodamente desde el móvil.

### 12.3 Principio de navegación
- En desktop: sidebar izquierda fija (como en los prototipos) + header superior.
- En móvil: header compacto + bottom nav.
- El foco táctil se mantiene generoso (mínimo 44×44 px).
...
### 15.1 Versiones (`json/versions.json`)
```json
{
  "versions": [
...
### 15.2 Catálogo de planes (`json/plans.json`)
```json
{
  "plans": [
    {
...
### Funciones diferenciadoras (Fase 3-4)
- **Sistema de logros:** Badges visuales por hitos (racha de 7/30/100 días, completar un libro, completar un testamento, etc.).
- **Reflexiones del Scriptorio:** Notas devocionales largas con fecha y referencia, tipo diario espiritual.
- **Contemplación semanal:** Quote o reflexión de un padre de la iglesia/santo, rotativa.
- **Reporte anual:** Exportación visual del progreso (PDF o imagen) con estadísticas de lectura.
...
### 18.1 Sistema de diseño oficial
El sistema de diseño completo está documentado en `prototipo/DESIGN.md` y visualizado en los prototipos HTML de la carpeta `prototipo/`:
- `bible_reader.html` — Visor de lectura bíblica
- `dashboard.html` — Dashboard / Inicio
- `my_progress.html` — Progreso y estadísticas
- `reading_plans.html` — Catálogo de planes

### 18.2 Paleta de colores (del prototipo)
- **Base:** `surface` `#fcf9f4` (pergamino), `on-surface` `#1c1c19` (carbón suave)
- **Primario:** `primary` `#4b6026` (verde oliva), `primary-container` `#63793c`
- **Secundario:** `secondary` `#725a38` (dorado/marrón), `secondary-container` `#fcdaaf`
- **Terciario:** `tertiary` `#68526c` (púrpura suave)
- **Acentos:** `primary-fixed` `#d2eca2` (lima suave para resaltados)
- **Dark mode:** Fondo `#1c1c19`, texto `#f0ede8`, acentos `#a8c878`

### 18.3 Tipografía (del prototipo)
- **La Voz (Escritura):** **Newsreader** (serif) para todo texto bíblico y titulares editoriales.
- **La Estructura (UI):** **Manrope** (sans) para navegación, etiquetas, metadatos y botones.
- **Ajuste óptico:** Letter-spacing `-0.02em` en display-lg para look editorial premium.

### 18.4 Reglas visuales clave (del prototipo)
- **No-Line Rule:** Prohibido usar bordes sólidos de 1px para seccionar. Separación por espacio negativo y cambios tonales de fondo.
- **Tonal Layering:** La UI es una serie de capas físicas (hojas de papel fino apiladas):
  - Nivel 1 (Base): `surface` `#fcf9f4`
  - Nivel 2 (Navegación/Contexto): `surface-container` `#f0ede8`
  - Nivel 3 (Elementos interactivos): `surface-container-high` `#ebe8e3`
- **Glass & Gradient:** UI flotante usa glassmorphism (`surface-container-lowest` 80% opacity + `24px` backdrop blur). CTAs primarios usan gradiente sutil `primary` → `primary-container` a 135°.
- **Ambient Shadows:** `0px 12px 32px rgba(28, 28, 25, 0.06)` — nunca negro puro.
- **Ghost Border:** Si se requiere límite por accesibilidad, usar `outline-variant` `#c8c7ba` al 15% de opacidad.

### 18.5 Componentes clave (del prototipo)
- **Buscador minimalista:** Línea única sin caja contenedora. Underline sutil `outline-variant` que se expande desde el centro al enfocar.
- **Tarjetas de planes:** Imágenes B&W por defecto, se colorean en hover. Sin bordes, separadas por espacio.
- **Calendario de consistencia:** Grilla de días tipo "GitHub contributions" con estados visuales (leído/no leído).
- **Gráfico de barras:** Versículos leídos por mes, barras con transiciones suaves.
- **Logros:** Tarjetas con icono, título, descripción y estado (desbloqueado/bloqueado).
- **Progress bar:** Barra delgada de 2px en `secondary` `#725a38` en la parte superior del viewport.
- **Resaltados de escritura:** Fondo `primary-fixed` `#d2eca2` con radio `0.125rem`, no marcador amarillo agresivo.

### 18.6 Mobile first (como web, no como app)
La mayoría del uso será móvil. Prioridades:
- Header sticky compacto.
- Bottom nav de 4 items en móvil (táctil, no app nativa).
- Botón grande de "Leer hoy" en portada.
- Texto bíblico con tipografía cómoda y línea de lectura amplia (`line-height: 1.8`).
- Controles táctiles generosos (mínimo 44×44 px).
- Modo oscuro disponible.
- La URL siempre visible para compartir fácilmente.

### 18.7 Principios de experiencia
...
### 18.8 Transiciones y animaciones
- **Duración:** 300ms–500ms para transiciones de opacidad y transform.
- **Easing:** Suave, "fade-and-slide", simulando pasar página. Nunca "bouncy" ni "snappy".
- **Hover en imágenes:** Grayscale → color, transición 700ms.
...
> **Mi Biblia 365** es una plataforma web responsive construida con Astro 6, que permite a los cristianos leer la Biblia mediante múltiples planes y versiones en español, apoyándose en una API bíblica externa con respaldo, datos JSON servidos por CDN, y un espacio personal completo para gestionar progreso, favoritos, notas y reflexiones devocionales. El proyecto no requiere registro en su primera fase, funciona directamente en el navegador sin pretender ser una app nativa, y está diseñado con el propósito pastoral de acompañar al creyente en su perseverancia devocional. Su identidad visual — paleta orgánica, tipografía editorial dual, espacio negativo generoso y capas tonales — evoca un scriptorium digital moderno.

---

## 22. Estructura de carpetas del repositorio

```
mibiblia365/
├── prototipo/                    # Prototipos HTML y sistema de diseño
│   ├── bible_reader.html
│   ├── dashboard.html
│   ├── my_progress.html
│   ├── reading_plans.html
│   ├── DESIGN.md                 # Sistema de diseño oficial
│   └── README.md                 # Documentación del prototipo
│
├── json/                         # Datos JSON (planes, versiones, versículos)
│   ├── versions.json
│   ├── plans.json
...
│   ├── plan-nuevos-creyentes.json
│   ├── plan-90-dias.json
│   ├── versiculos-diarios.json
│   └── config.json
│
├── src/                          # Código fuente Astro 6
│   ├── layouts/
│   ├── pages/
│   ├── components/
│   ├── lib/
...
7. **Construir `json/versiculos-diarios.json`** con 366 entradas siguiendo línea temática mensual.

---

*Documento de producto. Versión 3.0 — Web Responsive. Abril 2026.*
